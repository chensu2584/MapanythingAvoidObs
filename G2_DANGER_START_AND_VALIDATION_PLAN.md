...
规划中的末端碰撞模型门禁与重建体素去夹爪是两件不同的事。当前
`scripts/simplify_g2_snapshots.py` 对重建体素采用临时空间代理：

1. 从该 snapshot 的导出相机位姿读取左右手部相机中心；
2. 以每个相机中心为球心，计算每个体素中心的三维欧氏距离；
3. 删除距离任一相机中心小于 `--gripper-radius` 的体素，3box 当前默认半径为 `0.15 m`；
4. 对剩余点继续做 observation count、3D DBSCAN、桌面裁剪和基本体拟合。

它没有使用实装夹爪 mesh、手部朝向、机械臂 FK 包围体、语义分割、深度遮挡关系或体素的
来源视图。固定球形区域可能误删相机附近的真实物件，也可能保留球外的手指、腕部或机械臂；
当夹爪接触物件时尤其无法区分二者。`direct_depth_voxels.glb` 是原始深度对照，完全不执行
这一步。
...
1. 测量当前左右实装夹爪的保守几何及 `arm_end_T_gripper`/`arm_end_T_tcp`；
2. 用每帧关节状态做完整机械臂和实装夹爪 FK，生成逐 link 的 core 与 ambiguity shell；
3. 只自动删除同时满足“落入 robot core、来源视图相符、深度/遮挡一致”的体素；
4. ambiguity shell 默认保留并要求 GUI 审核，不再靠扩大固定球半径猜测边界；
5. 利用多 snapshot 中“场景静止、机器人随关节运动”的持续性分离机器人和场景；
6. 输出逐 link 删除数量、误删风险、前后 GLB、输入/模型哈希和 `self_filter_report.json`；
7. 未确认实装夹爪模型和 TCP 前，继续保持 `execution_authorized=false`。
